...
class SummaryTextViewController: SummaryViewController {
    ...
    override func viewDidLoad() {
        ...
        let amountSpentNumber = NSNumber(value: amountSpent ?? 0.0)
        let formattedAmountSpent = numberFormatter.string(from: amountSpentNumber)
        let attrAmountSpent = NSMutableAttributedString(string: " $" + (formattedAmountSpent ?? "") + " ")
        ...
        // Gets the total amount
        let totalAmountNumber = NSNumber(value: totalAmount ?? 0.0)
        let formattedAmount = numberFormatter.string(from: totalAmountNumber)
        let attrTotalAmount = NSMutableAttributedString(string: " $" + (formattedAmount ?? "") + " ")
